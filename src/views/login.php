...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'login') {
            $result = login_user($_POST['identifier'] ?? ($_POST['email'] ?? ''), $_POST['password'] ?? '');
            if ($result['success']) {
                $message = $result['message'];
                $message_type = 'success';
                // Don't redirect - show success message and refresh layout
...
<div class="max-w-md mx-auto mt-12 md:mt-24 animate-fade-in-up px-4">
    ...
    <div class="border border-neutral-800 bg-neutral-950 rounded-2xl p-8 shadow-2xl relative overflow-hidden group">
        ...
        <form method="POST" action="/login" class="space-y-5">
            ...
            <div>
                <label class="block text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">Username or Email</label>
                <div class="relative">
                    <input type="text" name="identifier" required autocomplete="username" placeholder="your_username or user@plexaur.com" class="w-full bg-black border border-neutral-800 rounded-lg py-3 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all placeholder-neutral-700">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" name="password" required placeholder="••••••••" class="w-full bg-black border border-neutral-800 rounded-lg py-3 pl-10 pr-4 text-white text-sm focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition-all placeholder-neutral-700">
                    <div class="absolute left-3 top-3 text-neutral-600">
        ...
        <div class="relative my-8">
            ...
            <div class="relative flex justify-center text-xs uppercase"><span class="bg-neutral-950 px-2 text-neutral-600">New Here?</span></div>
        </div>

        <!-- Footer -->
        <div class="text-center text-sm text-neutral-500">
            Don't have an account? <a href="/signup" class="text-white font-medium hover:underline">Create Account</a>
        </div>
    </div>
</div>
